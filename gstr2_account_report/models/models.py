...
class GSTR2L10nInReportAccount(models.AbstractModel):
    _name = "gstr2.l10n.in.report.account"
    ...
    def _get_options(self, previous_options=None):
        options = super(GSTR2L10nInReportAccount, self)._get_options(previous_options)
        options['gst_return_type'] = 'gstr2'
        options['gst_section'] = self.env.context.get('gst_section')
        return options

    def _set_context(self, options):
        ctx = super(GSTR2L10nInReportAccount, self)._set_context(options)
        ...
        if gst_section:
            ...
            for gst_section, gst_section_name in self.get_gst_section(gst_return_type).items():
                ...
                lines.append({
                    ...
                    'action_id': self.env.ref('gstr2_account_report.action_gstr2_account_report_menu').id,
                    'action': 'view_sub_type'
                    })
        return lines

    def get_html(self, options, line_id=None, additional_context=None):
        return super(GSTR2L10nInReportAccount, self.with_context(options or {})).get_html(options, line_id, additional_context)
    ...
    def get_gst_section(self, gst_return_type, gst_section=None):
        ...
        if gst_return_type == 'gstr2':
            gst_sections_list += [
                {'b2b': _("B2B Invoice")},
                # {'b2cl': _("B2C(Large) Invoice")},
                # {'b2cs': _("B2C(Small) Details")},
                {'b2bur': _("B2BUR")},
                {'cdnr': _("Credit/Debit Notes(Registered)")},
                {'cdnur': _("Credit/Debit Notes(Unregistered)")},
                # {'exp': _("Exports Invoice ")},
                {'at': _("Tax Liability(Advances Received)")},
                {'atadj': _("Adjustments of Advances")},
                {'hsn': _("HSN-wise Summary")},
                {'exemp': _("Summary For Nil rated, exempted and non GST outward supplies")}
                ]
    ...
    def get_gst_section_model_domain(self, gst_return_type, gst_section):
        ...
        sum_fields = [
            ...
            'sgst_amount']
        model = 'gstr2.l10n_in.account.invoice.report'
        if gst_return_type == 'gstr2':
            ...
            if gst_section == 'b2b':
                ...
                model = 'gstr2.l10n_in.advances.payment.report'
                domain = [
                    ('amount', '>', 0),
                    ('payment_type', '=', 'outbound')]
            elif gst_section == 'atadj':
                model = 'gstr2.l10n_in.advances.payment.adjustment.report'
                domain = [
                    ('payment_type', '=', 'outbound')]
            elif gst_section == 'hsn':
                model = 'gstr2.l10n_in.product.hsn.report'
            elif gst_section == 'exemp':
                sum_fields = ['account_move_id']
                model = 'gstr2.l10n_in.exempted.report'
                domain += [
                    ('in_supply_type', '!=', False),
                    '|', ('nil_rated_amount', '!=', 0),
                    '|', ('exempted_amount', '!=', 0),
                    ('non_gst_supplies', '!=', 0)]
        ...
        if gst_return_type == 'gstr2':
            if gst_section == 'b2b':
                section_fields += [
                    ...
                    {"name": "partner_vat", "label": "GSTIN/UIN of Supplier"},
                    {"name": "partner_id", "label": "Supplier Name"},
                ...
                section_fields = [
                    ...
                    {"name": "partner_id", "label": "Name"},
                    {"name": "reversed_entry_id", "label": "Invoice/Advance Receipt Number"},
                    {"name": "gst_format_refund_date", "label": "Invoice/Advance Receipt date"},
                    {"name": "refund_invoice_type", "label": "Document Type"},
                    {"name": "place_of_supply", "label": "Place Of Supply"},
                ...
                section_fields = [
                    ...
                    {"name": "partner_id", "label": "Composition Taxable person"},
