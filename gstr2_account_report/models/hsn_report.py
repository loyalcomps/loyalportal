...
class GSTR2L10nInProductHsnReport(models.Model):
    _name = "gstr2.l10n_in.product.hsn.report"
    ...
    def _select(self):
        select_str = """SELECT aml.id AS id,
            ...
            CASE WHEN tag_rep_ln.account_tax_report_line_id IN
                ...
                THEN aml.balance * (CASE WHEN aj.type = 'purchase' THEN -1 ELSE 1 END)
                ELSE 0
                END AS igst_amount,
            CASE WHEN tag_rep_ln.account_tax_report_line_id IN
            ...
            CASE WHEN tag_rep_ln.account_tax_report_line_id IN
                (SELECT res_id FROM ir_model_data WHERE module='l10n_in' AND name='tax_report_line_sgst')
                THEN 0
                ELSE (CASE WHEN aml.tax_line_id IS NOT NULL THEN aml.tax_base_amount ELSE aml.balance * (CASE WHEN aj.type = 'purchase' THEN -1 ELSE 1 END) END)
                END AS price_total,
            (CASE WHEN tag_rep_ln.account_tax_report_line_id IN
                (SELECT res_id FROM ir_model_data WHERE module='l10n_in' AND name='tax_report_line_sgst')
                THEN 0
                ELSE (CASE WHEN aml.tax_line_id IS NOT NULL THEN aml.tax_base_amount ELSE 1 END)
                END) + (aml.balance * (CASE WHEN aj.type = 'purchase' THEN -1 ELSE 1 END))  AS total
        """
        return select_str
