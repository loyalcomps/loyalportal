...
class GSTR2L10nInAdvancesPaymentAdjustmentReport(models.Model):
    _name = "gstr2.l10n_in.advances.payment.adjustment.report"
    ...
    def _compute_l10n_in_tax(self, taxes, price_unit, currency=None, quantity=1.0, product=None, partner=None):
        ...
        tax_compute = filter_tax.compute_all(price_unit, currency=currency, quantity=quantity, product=product,
                                             partner=partner)
        for tax_data in tax_compute['taxes']:
            tax_report_lines = AccountTaxRepartitionLine.browse(tax_data['tax_repartition_line_id']).mapped(
        ...
        return """FROM account_move_line aml
               ...
               JOIN account_tax AS tax ON tax.id = (
                   CASE WHEN ap.payment_type = 'outbound'
                       THEN c.account_purchase_tax_id
                       ELSE c.account_sale_tax_id END)
